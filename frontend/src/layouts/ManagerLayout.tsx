...
import { LayoutDashboard, CheckSquare, Users, BarChart2, LogOut, User } from 'lucide-react'
...
export default function ManagerLayout() {
  ...
  useEffect(() => {
    api.get('/escalations/my-team').then(r => setOpenEscalations(r.data.open_count)).catch(() => {})
  }, [])
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {nav.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to}
              ...
              {label === 'Dashboard' && openEscalations > 0 && (
